...
// TODO All fields below are supposed to be fetched form server.
quoridor.game.data = {}
quoridor.game.data.NONE = 'o'
quoridor.game.data.PLAYER = 'p'
quoridor.game.data.WALL = 'x'
quoridor.game.data.panelDataSize = 17
...
quoridor.game.data.playerNum = 2 // player number of current game
quoridor.game.data.currentIndex = 0 // index of player in action
quoridor.game.data.myIndex = 0 //Index of player queue
quoridor.game.data.startInfos = new Array() // start info(position & walls) of all players, order by index
quoridor.game.data.currentInfos = new Array() // current info(position & walls) of all players, order by index
quoridor.game.data.myWalls = 10
...
quoridor.game.init = function(w, h, id) {
	...
	// Init data map
	var size = this.data.panelDataSize
	var data = this.data.panelData = new Array(size)
	for (var i = 0; i < size; i++) {
		data[i] = new Array(size)
		for (var j = 0; j < size; j++) {
			data[i][j] = this.data.NONE
		}
	}

	function preload() {
		//		// Init socket
		//		quoridor.socket.create({
		//			open: function() {
		//				var session = quoridor.cookie.get("session")
		//				console.log("register with session", session)
		//				quoridor.socket.sendMessage(quoridor.cmd.Register, session, false)
		//			},
		//			message: function(json) {
		//				if (json.confirm == true) {
		//					// If it is a confirm message, ignore.
		//					quoridor.handler.onConfirm(json)
		//				} else {
		//					// If not a confirm message, handle it & confirm.
		//					quoridor.handler.onMessage(json)
		//					json.confirm = true
		//					quoridor.socket.send(json)
		//				}
		//			},
		//			close: function() {
		//				console.log("close")
...
quoridor.game.onPotentialClick = function(sprite) {
	...
	//TODO sync server with wall position
	quoridor.test.nextPlayer()

	//TODO 
	for (var i = 0; i < 17; i++) {
		console.log(this.data.panelData[i])
	}
}
			...
			for (var j = 0; j < 4; j++) {
				...
				if (cellx == x || celly == y) {
					...
					if (this.isPlayer(cellx, celly)) {
						console.log("has player ahead, noway, clear pending")
						pending.length = 0
						break
					} else if (this.isEmpty(cellx, celly)) {
...
quoridor.game.createGamePanel = function(game) {
	...
	//Player texture
	// TODO remove test data, use data from server.
	var points = [{
		x: 8,
		y: 16
	}, {
	...
	for (var i = 0; i < playerNum; i++) {
		...
		// TODO remove test data, use data from server.
		this.data.startInfos.push({
			x: point.x,
			y: point.y
		})
		this.data.currentInfos.push({
			x: point.x,
			y: point.y
		})
	}
...
quoridor.test = {}
quoridor.test.nextPlayer = function() {
	quoridor.game.data.myIndex += 1
	quoridor.game.data.myIndex %= quoridor.game.data.playerNum

	quoridor.game.data.currentIndex += 1
	quoridor.game.data.currentIndex %= quoridor.game.data.playerNum
